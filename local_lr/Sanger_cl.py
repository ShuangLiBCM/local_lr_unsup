...
# Define Sanger's local learning rule
...
def sanger(eta=0.01, act=None):
    """A fully connected linear layer with Sanger's rule."""
    ...
    def update(x, y, **kwargs):
        ...
        for ii in np.arange(D):
            dw[:, ii] = R.T.dot(y[:, ii]) - w[:, ii] * y[:, ii].T.dot(y[:, ii])
            R = R - y[:, ii].reshape(-1,1).dot(w[:, ii].reshape(-1,1).T)
